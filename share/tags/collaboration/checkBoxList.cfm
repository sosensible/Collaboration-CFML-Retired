...
<cfswitch expression="#thisTag.executionMode#">
	<cfcase value="start">
		...
		<cfparam name="attributes.fieldItemClass" default="coCheckBox">
		...
		<cfset root.checkBoxList = 'Array,String,Query,Structure'>
		<cfif attributes.id EQ ''>
			<cfthrow detail="Missing required attribute: 'id'" message="'id' attribute is required">
		</cfif>
		<cfscript>
		standardAttributes = 'fieldParam,formatter,formatterMask,rowEnd,rowStart,rowMaxItems,fieldItemClass,fieldGroupClass,formatterType,formatterVersion,orientation,innerAttributes,id,name,data,valueField,displayField,label,labelClass,delimiters,idType,selected,editModes,showModes,allowBreak,viewModes';
	...
	<cfcase value="end">
		...
		for(iRow=1; iRow LTE recordCount; iRow = iRow + 1) {
			...
			if (editMode) {
				formElement = formElement & '
						...
						<input#attributeList# type="checkBox" id="#thisId#" name="#attributes.name#" value="#value#"#isChecked##innerAttributeList##innerDataAttributeList#/>
						<label for="#thisID#">#display#</label>
						</span>';
		...
		if(!len(trim(checkedDisplayList))){ checkedDisplayList = "&nbsp;"; }
		editModeMarkup = '<div id="#attributes.id#">#formElement#</div>';
